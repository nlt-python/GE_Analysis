...
ge_html = requests.get("https://catalog.ucmerced.edu/preview_program.php?catoid=16&poid=1986")
...
cleaned_ges = [x.replace("/a", "").replace("\xa0", " ") for x in ge_classes_txt if regex.match(x)]
...
#    print(idx, itm)
ge_dict = {"Social Science Courses": cleaned_ges[:32], "Arts and Humanities Courses": cleaned_ges[32:]}
#print(ge_dict)
...
def scraped_badge_classes(some_url):
    ...
    cleaned_rows = [row.strip("\t").replace("/a", "").replace("\xa0", " ") for row in rows if ": " in row]
    regex = re.compile("[A-Z]{2,4}\s[0]{1}[0-9]{2}:")
...
ss_yes_badges, ss_no_badges = xref_classes("ss_yes", "ss_no", ge_dict["Social Science Courses"], badges_dict)
ah_yes_badges, ah_no_badges = xref_classes("ah_yes", "ah_no", ge_dict["Arts and Humanities Courses"], badges_dict)
...
ge_badges_dict = {"Social Science in Badges": ss_yes_badges, "Arts and Humanities in Badges": ah_yes_badges}
ge_no_badges_dict = {"Social Science not in Badges": ss_no_badges, "Arts and Humanities not in Badges": ah_no_badges}
